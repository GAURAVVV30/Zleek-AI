...
export default function AssessmentPage() {
  ...
  if (isLoading || !assessment) {
    return (
      <div className="py-20 text-center">
        ...
        <p className="text-xs text-slate-500">Preparing concept assessment...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      {result ? (
        <div className="bg-white border border-slate-200/80 rounded-3xl p-8 shadow-card text-center space-y-6 animate-in zoom-in-95 duration-200">
          ...
          <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 text-xs text-slate-700">
            Score: <strong className="text-slate-900 font-bold">{result.scorePercentage}%</strong> · Evidence verified & logged to audit record.
          </div>

          <button
            onClick={() => navigate('/roadmap')}
            className="px-8 py-3.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl text-xs shadow-elevated transition inline-flex items-center gap-2"
          >
        ...
        <div className="bg-white border border-slate-200/80 rounded-3xl p-6 sm:p-8 shadow-card space-y-6">
          <div className="flex items-center justify-between pb-4 border-b border-slate-100">
            <div>
              ...
              <h1 className="text-lg font-bold text-slate-900">{assessment.conceptTitle} — Assessment</h1>
            </div>
          ...
          <div className="space-y-6">
            {assessment.questions?.map((q) => (
              <div key={q.id} className="p-5 bg-slate-50/60 rounded-2xl border border-slate-200/80 space-y-3">
