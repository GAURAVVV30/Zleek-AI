...
export default function BaselineResultsPage() {
  ...
  useEffect(() => {
    const sessionId = localStorage.getItem('diagnosticSessionId');
    if (!sessionId) {
      setIsLoading(false);
      return;
    }
    apiClient
      .get(ENDPOINTS.DIAGNOSTIC.RESULTS(sessionId))
      .then((res) => {
        setResults(res.data);
        setIsLoading(false);
      })
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 py-20 text-center">
        ...
        <p className="text-xs text-slate-400">
          {isLoading
            ? 'Synthesizing concept gaps & prerequisite graph...'
            : 'No diagnostic baseline found. Continue to your personalized roadmap.'}
        </p>
        {!isLoading && !results && (
          <button
            onClick={handleGenerateRoadmap}
            disabled={isGenerating}
            className="mt-6 px-8 py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold text-xs shadow-elevated hover:shadow-glow transition flex items-center gap-2 mx-auto"
          >
